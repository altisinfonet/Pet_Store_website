...
const NewArrivals = ({ newMenuArrData, metaDataData, URL, meta_description, meta_key }: any) => {

  const router = useRouter()
  const { get_arrival } = getUrlWithKey("client_apis")
  const { tabView, mobView } = useTabView()

  console.log(metaDataData, 'metaDataData')

  const [productPagetion, setProductPagetion]: any = useState({ page: 1, rowsPerPage: 10, all: true })
  const [productData, setProductData]: any = useState([])
  const [more, setMore] = useState(false)
  // const [products, setProducts]: any = useState()

  const { sendData: getArrival, loading: productsLoading, rawData: productsRaw }: any = useRead({ selectMethod: "put", url: get_arrival, callData: productPagetion });
  ...
  useEffect(() => {
    if (getArrival?.products?.length && more === false) {
      setProductData(getArrival?.products.map((v: any) => v?.product_details))
    }
    if (productsRaw?.success && more === true && getArrival?.previous_page === productPagetion?.page) {
      setProductData((pre: any) => pre?.length ? [...pre, ...getArrival?.products.map((v: any) => v?.product_details)] : getArrival?.products.map((v: any) => v?.product_details))
    }
  }, [getArrival, more])
  ...
  return (
    <>
      <MetaHead meta_title={`${pmetaDataData?.meta_data?.meta_title}`} meta_description={meta_description} keywords={meta_key} />
      {/* <MetaHead meta_title="new arrivals" meta_description="new arrivals" /> */}
      {!false ? <ProductListing
