...
const OTPTemplate = () => {
    ...
    const submitTemplate = async () => {
        ...
        try {
            let valid = false;
            valid = validation(formTemplateState, ["subject", "content"], "_OTPTEMPLATE");

            if (valid) {
                if (formTemplateState?.id) {
                    let metadata: any = { ...formTemplateState, content: pageDescription };
                    delete metadata.id;
                    let data = await axios.post(`${a_update}`, { setting_id: "_OTPTEMPLATE", id: formTemplateState.id, metadata: JSON.stringify(metadata) })
                    if (data?.data?.success) {
                        getFormTemplateSetting()
                        _SUCCESS("OTP Template Setting Updated Successfully!");
                    }
                } else {
                    let data = await axios.post(`${a_create}`, { setting_id: "_OTPTEMPLATE", metadata: JSON.stringify({ ...formTemplateState, content: pageDescription }) })
                    if (data?.data?.success) {
                        getFormTemplateSetting();
    ...
    return (
        <div>
            <SimpleCard heading={"OTP Template"}>
                <>
