...
const Offer = ({ metaDataData, newMenuArrData, URL, meta_description, meta_key }: any) => {
  const router = useRouter()
  const { get_product_offer_list } = getUrlWithKey("client_apis")
  ...
  const { sendData: getOffer, loading: productsLoading, rawData: productsRaw }: any = useRead({ selectMethod: "put", url: get_product_offer_list, callData: productPagetion });
  ...
  useEffect(() => {
    if (getOffer?.products?.length && more === false) {
      setProductData(getOffer?.products)
    }
    if (productsRaw?.success && more === true && getOffer?.previous_page === productPagetion?.page) {
      setProductData((pre: any) => pre?.length ? [...pre, ...getOffer?.products] : getOffer?.products)
    }
  ...
  return (
    <>
      <MetaHead meta_title={pmetaDataData?.meta_data?.meta_title} meta_description={meta_description} keywords={meta_key} />
      {/* <MetaHead meta_title="offer" meta_description="offer" /> */}
