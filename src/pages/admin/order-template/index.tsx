...
const OrderTemplate = () => {
    ...
    const submitTemplate = async () => {
        ...
        try {
            let valid = false;
            valid = validation(formTemplateState, ["subject", "content"], "_ORDERTEMPLATE");

            if (valid) {
                if (formTemplateState?.id) {
                    let metadata: any = { ...formTemplateState, content: pageDescription };
                    delete metadata.id;
                    let data = await axios.post(`${a_update}`, { setting_id: "_ORDERTEMPLATE", id: formTemplateState.id, metadata: JSON.stringify(metadata) })
                    if (data?.data?.success) {
                        getFormTemplateSetting()
                        _SUCCESS("Order Template Setting Updated Successfully!");
                    }
                } else {
                    let data = await axios.post(`${a_create}`, { setting_id: "_ORDERTEMPLATE", metadata: JSON.stringify({ ...formTemplateState, content: pageDescription }) })
                    if (data?.data?.success) {
                        getFormTemplateSetting();
    ...
    return (
        <div>
            <SimpleCard heading={"Order Template"}>
                <>
