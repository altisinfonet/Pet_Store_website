...
const NotifyMePoster = () => {
    ...
    const fetchFormTemplateSetting = async () => {
        try {
            let { data } = await axios.get(`${a_get_with_id}/_NOTIFYMEPOSTER`)
            if (data?.success) {
                if (data?.data) {
    ...
    const submitTemplate = async () => {
        ...
        try {
            let valid = false;
            valid = validation(formTemplateState, ["subject", "content"], "_NOTIFYMEPOSTER");

            if (valid) {
                if (formTemplateState?.id) {
                    let metadata: any = { ...formTemplateState, content: pageDescription };
                    delete metadata.id;
                    let data = await axios.post(`${a_update}`, { setting_id: "_NOTIFYMEPOSTER", id: formTemplateState.id, metadata: JSON.stringify(metadata) })
                    if (data?.data?.success) {
                        getFormTemplateSetting()
                        _SUCCESS("Notify Me Poster Template Setting Updated Successfully!");
                    }
                } else {
                    let data = await axios.post(`${a_create}`, { setting_id: "_NOTIFYMEPOSTER", metadata: JSON.stringify({ ...formTemplateState, content: pageDescription }) })
                    if (data?.data?.success) {
                        getFormTemplateSetting();
                        _SUCCESS("Notify Me Poster Template Setting Created Successfully!");
                    }
                }
            }
        } catch (error) {
    ...
    return (
        <div>
            <SimpleCard heading={"Notify Me - Poster"}>
                <>
